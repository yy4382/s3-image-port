---
title: Cloudflare R2 逐步指南
---

<Callout>
  Cloudflare R2 Storage
  允许开发人员存储大量非结构化数据，而无需支付与典型云存储服务相关的昂贵出口带宽费用。
</Callout>
...
- `Endpoint`: 参见 "查看 Endpoint" 部分
- `Bucket Name`: 您自定义的存储桶名称，参见 "创建存储桶" 部分
- `Region`: 默认应该为 `auto`，参见 "创建存储桶" 部分
- `Access Key ID`: 参见 "获取密钥" 部分
- `Secret Access Key`: 参见 "获取密钥" 部分
- `Public URL`: 参见 "公开您的存储桶" 部分
